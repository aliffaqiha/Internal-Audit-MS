...
export function CapsPage() {
  ...
  const handleVerify = (cap: CorrectiveActionDto) => {
    if (!confirm(`Setujui dan tutup CAP "${cap.id}"?`)) return
    run(() => capsApi.verify(cap.id, { approve: true, note: null }))
  }

  const handleReject = (cap: CorrectiveActionDto) => {
    const reason = prompt("Alasan penolakan (CAP akan dibuka kembali untuk perbaikan)?", "")
    if (reason === null) return
    run(() => capsApi.verify(cap.id, { approve: false, note: reason || null }))
  }
